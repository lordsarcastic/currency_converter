...
from .schema import ConversionResponseSchema, ConvertSchema, CurrencyListSchema, CurrencySchema, GetHistorySchema, HistorySchema
...
def craft_conversion_urls(_from, to):
    urls = list(map(
        lambda url: url.format(_from, to),
        CONVERSION_APIS
    ))
...
class ConverterService(BaseService):
    ...
    def convert(cls, payload: ConvertSchema) -> ConversionResponseSchema:
        ...
        rate = make_request_with_retries(conversion_urls).get(
            payload.to_currency
        )

        result = ConversionResponseSchema(
            rate=rate,
            result=rate * payload.amount,
            **payload.to_dict()
        )
    ...
    def store_conversion_to_history(
        self,
        payload: ConversionResponseSchema,
        user: UUID
    ):
        history_to_db = ConversionHistory(
            **payload.to_dict(),
            user_id=user
        )
        self.save(history_to_db)
    
    def get_conversion_history(self, payload: GetHistorySchema, user: User) -> List[ConversionHistory]:
        histories = self.db.query(ConversionHistory).filter(
            ConversionHistory.user == user
        )
        if (currency := payload.from_currency):
            histories = histories.filter(
                ConversionHistory.from_currency == currency
            )
        if (currency := payload.to_currency):
            histories = histories.filter(
                ConversionHistory.to_currency == currency
            )
